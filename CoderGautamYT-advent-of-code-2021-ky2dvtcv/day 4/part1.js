...
for (var i = 0; i < boardArray.length; i++) {
   ...
   // board.splice(board.length - 1, 1);
    boardArray[i] = board
    randarrayidk[i] = board;
}

//ok now its bingo time

var boardArrayUse = [].concat(boardArray);
...
numbers.forEach(function(number) {
    if(stop) return;
    //check each board
    for (var i = 0; i < boardArrayUse.length; i++) {
        var board = boardArrayUse[i]
        //check each row
        for (var j = 0; j < board.length; j++) {
            var row = board[j]
