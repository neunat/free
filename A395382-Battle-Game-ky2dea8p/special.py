...
# Hisirdoux class
class Hisirdoux(Player):
  def __init__(self):
    Player.__init__(self, "Hisirdoux")
...
# special move:
  ...
  def pika_block(self, enemy): # blocks 25 of enemy's attack
    print("Hisirdoux will get 20 health to block some of the enemy's attack.")
    self.health += 20
    time.sleep(1)
  ...
  def raichu_block(self, enemy):
    ...
    if self.specialmove > 0:
      ...
      time.sleep(1)
      print("You cannot use this move anymore.")
      self.moves.pop("raichu block") # remove from dict of possible moves
