class Player:
  def __init__(self, name, health, maxHealth, money, level, xp, color, head, chest, legs, feet, bag):
    self.name = name
    self.health = health
...
class Enemy:
  def __init__(self, enemyName, referenceName, enemyType, prefix, description, color, armor, speed, damage, health, maxHealth, isBoss, lootTable):
    self.name = enemyName
    self.reference_name = referenceName
...
enemyData = {
  "greenSlime":Enemy("Green Slime","greenSlime","slime","a","A variant of slime with green coloring. More akin to plant than creature, some believe they have adapted to capture energy from the sun. Because of this, they are fairly weak.","\033[m",0,1,1,5,5,False, [0,4,[]]),
}