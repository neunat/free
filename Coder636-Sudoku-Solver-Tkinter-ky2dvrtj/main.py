...
for i in range(9):
    for j in range(9):
        if (i in (0, 1, 2, 6, 7, 8) and j in range(3, 6)) or (j in (0, 1, 2, 6, 7, 8) and i in range(3, 6)):
            e = Entry(root, width=1, font=('ink free', 18, 'bold'), bg='sky blue', fg='blue', justify='center', cursor='center_ptr', insertbackground='blue', borderwidth=1)
            e.grid(row=i, column=j, ipadx=12, ipady=5)
        else:
            e = Entry(root, width=1, font=('ink free', 18, 'bold'), fg='green', justify='center', cursor='center_ptr', insertbackground='blue', borderwidth=1)
            e.grid(row=i, column=j, ipadx=12, ipady=5)
        grid[i].append(e)
