...
class GameScene extends Phaser.Scene {
        ...
        grecaptcha.ready(() => {
            grecaptcha.execute("6LdVxgYdAAAAAPtvjrXLAzSd2ANyzIkiSqk_yFpt", { action: "join" }).then((thetoken) => {
                ...
                try {
                    ...
                    //chat
                    this.chatDisplay = this.add.text(10, 110, "", {
                        fontFamily: "Georgia, \"Goudy Bookletter 1911\", Times, serif"
                    }).setFontSize(15).setDepth(101);
                    //this.chatDisplay.setScrollFactor(0);
                } catch (e) {
                    console.log(e);
                }
                //minimap
                ...
                this.cameras.main.ignore([this.killCount, this.playerCount, this.leaderboard, this.chatDisplay, this.lvlBar.bar, this.lvlText, this.lvlState]);
                ...
                this.socket.on("coins", (coinsArr) => {
                    ...
                    coinsArr.forEach((coin) => {
                        if (this.coins.filter(e => e.id == coin.id).length == 0) addCoin(coin);
                    });
                ...
                this.socket.on("coin", (coin, start) => {
                    if (Array.isArray(coin)) {
                        if (start) coin.forEach((x) => addCoin(x, start));
                        else coin.forEach((x) => addCoin(x));
                    } else addCoin(coin);
                });
    ...
    update(time, delta) {
        ...
        if (!enterKey.isDown) this.chatLock = false;
        if (enterKey.isDown && !this.chatLock) {
            this.chatLock = true;
            var message = prompt('');
            if (message) this.socket.emit('send_chat', message)
        }
        ...
        try {
            ...
            if (this.key.up.isDown || wKey.isDown) controller.up = true;
            if (this.key.down.isDown || sKey.isDown) controller.down = true;
            if (this.key.right.isDown || dKey.isDown) controller.right = true;
            if (this.key.left.isDown || aKey.isDown) controller.left = true;
 ...
 //without lerp
        ...
        */
        ...
        //chat display
        try {
            let text = JSON.parse(JSON.stringify(this.chat)).reverse()
                .map(x => ` ${x.sender} - ${x.content}`.match(/.{1,26}/g)
                    .join('\n')).join('\n');
            if (text.split('\n').length > 15) text = text.split('\n').reverse().splice(0, 15).reverse().join('\n');
