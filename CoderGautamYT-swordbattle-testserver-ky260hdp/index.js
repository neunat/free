...
var oldlevels = [
  ...
  {coins: 40000, scale: 4.4},
];
var levels = [];
oldlevels.forEach((level, index)  =>{
	if(index == 0) levels.push(Object.assign({start: 0},level)); 
	else levels.push(Object.assign({start: levels[index - 1].coins}, level));
});
...
app.get("/leaderboard", async (req, res) => {
	...
	if(duration != "all") var lb = await sql`SELECT * from games where EXTRACT(EPOCH FROM (now() - created_at)) < ${duration == "day" ? "86400" : "608400"} ORDER BY ${ sql(type) } DESC LIMIT 23`;
	else var lb = await sql`SELECT * from games ORDER BY ${ sql(type) } DESC LIMIT 23`;
...
io.on("connection", async (socket) => {
	...
	if (socket.handshake.xdomain) socket.disconnect();

    socket.on("send_chat", (message) => {
        let msg = {};
        msg.sender = PlayerList.getPlayer(socket.id).name;
        msg.id = Date.now() + (Math.floor(Math.random()*89999999999) + 10000000000);
        msg.content = message;
        chat.unshift(msg);
        if(chat.length > 15) chat.length = 15;
        console.log(msg);
    })
