...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if (!message.member.hasPermission("MANAGE_CHANNELS")) return message.channel.send(":x: You Don't Have Enough Permission To Execute This Command - Manage Channels");
    
    let Channel = message.mentions.channels.first() || message.guild.channels.cache.get(args[0]);
    
    if (!Channel || Channel.type === "voice") return message.channel.send(`:x: Please Give A Valid Text Channel!`);
    ...
    if (!Type || !Types.find(T => T === Type.toLowerCase())) return message.channel.send(`:x: Please Give A Valid Type - Welcome, Wel, Join, Leave, Left`);
